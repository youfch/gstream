...
## gStream Bidirectional Demo — 双向流测试
## Godot 推送画面到浏览器，同时接收浏览器的摄像头视频
## 验证：WebRTC SendRecv 模式 + 远端视频渲染
...
func connect_stream_server() -> void:
	...
	if ss and ss.has_signal("ClientConnected"):
		...
		if ss.has_signal("RemoteVideoReady"):
			ss.connect("RemoteVideoReady", _on_remote_video_ready)
...
func _on_remote_video_ready(width: int, height: int) -> void:
	_update_status("远端视频就绪 %dx%d" % [width, height])
	_flash_color(Color.CYAN)
