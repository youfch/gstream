...
function showWarningIfNeeded(startupMode) {
  ...
  if (startupMode == "public") {
    warningDiv.innerHTML = `<h4>${t('warning')}</h4> ${t('warning.public')}`;
...
async function setUp() {
  ...
  session.onConnect = () => {
    ...
    for (const track of tracks) {
      session.addTransceiver(track, { direction: 'sendonly' });
    }
    setCodecPreferences();
    showStatsMessage();
  };
  ...
  await session.start();
  await session.createConnection(connectionId);
}
...
async function hangUp() {
  ...
  setupButton.disabled = false;
  await session.deleteConnection();
  await session.stop();
  session = null;
...
function showStatsMessage() {
  intervalId = setInterval(async () => {
    if (localVideo.videoWidth) {
      localVideoStatsDiv.innerHTML = `<strong>${t('sending.resolution')}</strong> ${localVideo.videoWidth} x ${localVideo.videoHeight} px`;
    }
    if (remoteVideo.videoWidth) {
      remoteVideoStatsDiv.innerHTML = `<strong>${t('receiving.resolution')}</strong> ${remoteVideo.videoWidth} x ${remoteVideo.videoHeight} px`;
    }
