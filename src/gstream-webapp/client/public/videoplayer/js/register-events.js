/**
 * gStream VideoPlayer Input Event Registration
 * Registers mouse, keyboard, touch, and gamepad event handlers
 * for the videoplayer mode. Uses a simpler binary format than sender.js
 * (direct event encoding without the full input remoting pipeline).
 */
...
// Event type codes for the videoplayer binary protocol
const InputEvent = {
  Keyboard: 0,
  Mouse: 1,
  MouseWheel: 2,
...
let sendGamepadButtonPressed = undefined;
...
export function registerGamepadEvents(videoPlayer) {
  ...
  gamepadAxisChange = (e) => {
    ...
    const data = new DataView(new ArrayBuffer(27));
    data.setUint8(0, InputEvent.Gamepad);
...
export function registerKeyboardEvents(videoPlayer) {
  ...
  function sendKey(e, type) {
    ...
    Logger.log("key " + (type === KeyboardEventType.Down ? "down" : "up") + " " + key + ", repeat = " + e.repeat + ", character = " + character);
    vp && vp.sendMsg(new Uint8Array([InputEvent.Keyboard, type, e.repeat, key, character]).buffer);
  }

  sendKeyUp = (e) => sendKey(e, KeyboardEventType.Up);
  sendKeyDown = (e) => sendKey(e, KeyboardEventType.Down);
...
export function registerMouseEvents(videoPlayer, playerElement) {
  ...
  function sendTouch(e, phase) {
    ...
    // Merge changed touches into all touches
    for (let i = 0; i < changedTouches.length; i++) {
      if (!touches.find(t => t.identifier === changedTouches[i].identifier)) {
        touches.push(changedTouches[i]);
      }
    }

    for (let i = 0; i < touches.length; i++) {
      phases[i] = changedTouches.find(t => t.identifier === touches[i].identifier)
        ? phase
        : PointerPhase.Stationary;
    }

    Logger.log("touch phase:" + phase + " length:" + changedTouches.length + " pageX:" + changedTouches[0].pageX + ", pageY: " + changedTouches[0].pageY + ", force:" + changedTouches[0].force);
    ...
    for (let i = 0; i < touches.length; i++) {
      const scale = vp.videoScale;
      const originX = vp.videoOriginX;
      const originY = vp.videoOriginY;

      const x = (touches[i].pageX - originX) / scale;
      const y = vp.videoHeight - (touches[i].pageY - originY) / scale;

      data.setInt32(offset, touches[i].identifier, true); offset += 4;
      data.setUint8(offset, phases[i]); offset += 1;
      data.setInt16(offset, x, true); offset += 2;
      data.setInt16(offset, y, true); offset += 2;
      data.setFloat32(offset, touches[i].force, true); offset += 4;
    }

    vp && vp.sendMsg(data.buffer);
  }

  sendTouchMove = (e) => { sendTouch(e, PointerPhase.Moved); e.preventDefault(); };
  sendTouchStart = (e) => { sendTouch(e, PointerPhase.Began); e.preventDefault(); };
  sendTouchEnd = (e) => { sendTouch(e, PointerPhase.Ended); e.preventDefault(); };
  sendTouchCancel = (e) => { sendTouch(e, PointerPhase.Canceled); e.preventDefault(); };
  ...
  sendMouse = (e) => {
    ...
    const y = vp.videoHeight - (e.clientY - originY) / scale;
  ...
  // Touch events (W3C Touch Events Level 1/2)
...
/**
 * Send a button click event to the engine.
 * @param {VideoPlayer} videoPlayer
 * @param {number} elementId - Button identifier (1=blue, 2=green, 3=orange)
 */
